...
function CartItems({ cart }) {
  ...
  return (
    <div className="grid w-full grid-rows-[2fr_1fr_1fr] items-center border-b-2 pt-6 pb-6 md:grid-cols-[2fr_1.5fr_0.5fr] md:border-0 md:pb-0 lg:h-[10rem]">
      ...
      <div className="mt-4 justify-self-end pr-8 text-xl md:mt-0 md:text-3xl">
        <div className="flex gap-20 md:gap-8">
          <p>${cart.price.toFixed(2)}</p>
          <button
            onClick={handleRemove}
            className="cursor-pointer border-2 px-2.5 text-lg font-semibold hover:bg-red-600 hover:text-yellow-50 active:bg-red-600 active:text-yellow-50"
          >
            DELETE
          </button>
        </div>
